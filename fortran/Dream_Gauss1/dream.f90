...
!***  Compute log likelihood function 
   ...
   !integer::m num_gen
      ...
      do i=1,ob_dim
          s(i)=0
          do j=1,ob_dim
              s(i)=(ob_data(j)-m_comp(j,r,m))*invercov(i,j,r,m)+s(i)
          end do
          Determ_2=Determ_2+(ob_data(i)-m_comp(i,r,m))*s(i)
      end do
      fit(r,m)=-1.0/2.0*Determ_1-1.0/2.0*log(Determ_2)-ob_dim/2.0*log(2*pi)
...
subroutine comp_pre(r,m)!计算预测值子程序
 implicit none
 integer::r,m
 m_pre(:,r,m)=cm(Z(:,r,m),yo(:,:))!dim_pre维
 end subroutine
 ...
 real::var_b(dim_pre,dim_pre)!dim_pre*dim_pre维，协方差矩阵
 real(kind=8)::b(dim_pre)!dim_pre维
 integer::seed,i
 av_b=avbfun(r,m)
 var_b=varbfun(r,m)
 call multinormal_sample(dim_pre,real(var_b,kind=8),real(av_b,kind=8),seed,b)!得到结构误差b，dim_pre维
 do i=1,dim_pre
 ...
 real::varbfun(dim_pre,dim_pre)!dim_pre*dim_pre维
 real::s(ob_dim)
 integer::i,j,k,n,r,m
 !varbfun
 do j=1,dim_pre
